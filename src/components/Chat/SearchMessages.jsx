      ...
      <div className="overflow-auto custom-scrollbar h-full ">
        ...
        <div className="flex justify-center h-full flex-col">
          {searchTerm.length > 0 && !searchedMessages.length && (
            <span className="w-full text-secondary flex justify-center">
              No messages found...
            </span>
          )}
          <div className="flex flex-col w-full h-full">
            {searchedMessages.map((message) => (
              <div
                key={message?.id}
                className="flex flex-col cursor-pointer justify-center hover:bg-background-default-hover w-full px-5 border-b-[0.1px] border-secondary py-5"
              >
                ...
                <div className="text-icon-green">
                  {message?.message}&nbsp;&nbsp;
                  {message?.senderId === userInfo?.id && "(you)"}
                </div>
              </div>
            ))}
          </div>
